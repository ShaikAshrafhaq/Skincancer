...
export function AuthProvider({ children }) {
  ...
  const registerAccount = async (name, email, password) => {
    ...
    try {
      ...
      const response = await apiService.register({ 
        ...
        last_name: lastName,
        password,
        password_confirm: password // Confirm password for backend validation
      });
  ...
  const requestNewOtp = async () => {
    ...
    try {
      ...
      const response = await apiService.resendOTP(pendingUser.email);
      
      if (response.otp_code) {
        // Update pending user with new OTP from backend
        setPendingUser({ ...pendingUser, otp: response.otp_code });
        try { window.alert(`Your OTP code is: ${response.otp_code}`); } catch (_) {}
        return { success: true };
      } else {
        // If backend doesn't return otp_code, fallback to local generation
        const otp = generateSixDigitOtp();
      ...
      console.warn('Backend resend failed, using local fallback:', error);
      const otp = generateSixDigitOtp();
      setPendingUser({ ...pendingUser, otp });
      try { window.alert(`Your OTP code is: ${otp}`); } catch (_) {}
      return { success: true };
