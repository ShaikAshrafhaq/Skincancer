...
export default function OtpModal({ isOpen, email, currentOtp, onVerify, onClose, onResend, autoRefreshSeconds = 30 }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setSecondsLeft((s) => {
        if (s <= 1) {
          // Just reset the timer, don't auto-resend
        ...
        <div className="actions" style={{ justifyContent: "space-between", marginTop: 12 }}>
          ...
          {onResend && (
            <button 
              ...
              style={{ 
                opacity: (resendLoading || secondsLeft > 0) ? 0.6 : 1,
                cursor: (resendLoading || secondsLeft > 0) ? 'not-allowed' : 'pointer'
              }}
            >
              {resendLoading ? "Sending..." : secondsLeft > 0 ? `Resend (${secondsLeft}s)` : "Resend code"}
            </button>
          )}
        </div>
      </div>
